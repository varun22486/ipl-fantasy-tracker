...
function allApiKeys(): string[] {
  return [
    ...
    cleanEnvText(process.env.CRICKET_API_KEY_12),
...
async function fetchJson(path: string) {
  ...
  if (keys.length === 0) {
    throw new Error("Cricket API error: no CRICKET_API_KEY / CRICKET_API_KEY_2…_12 configured");
  }
...
function extractMomFromFreeText(text: string): string | null {
  ...
  const patterns = [
    ...
    // Require \b after the acronym so "mom" inside "moments" does not match.
